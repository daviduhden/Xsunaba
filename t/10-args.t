...
$o = ok_args(
    'full args',              '--parent-display',
    ':0.0',                   '--parent-xauth',
    '/tmp/weird/.Xauthority', '--display',
    '50',                     '--width',
    '1280',                   '--height',
    '900',                    '--unveil',
    '/tmp:rwc,/etc:r',        '--amnesiac',
    '--verbose',              '--',
...
bad_args( 'too many app args',
    @$base, '--', '/bin/true', ( map { "a$_" } 1 .. 260 ) );
bad_args(
    'parent display host form', '--parent-display',
    'host:0',                   '--parent-xauth',
    '/tmp/a',                   '--',
    '/bin/true'
);
bad_args(
    'parent display localhost', '--parent-display',
    'localhost:0',              '--parent-xauth',
    '/tmp/a',                   '--',
    '/bin/true'
);
bad_args(
    'parent display empty', '--parent-display',
    '',                     '--parent-xauth',
    '/tmp/a',               '--',
    '/bin/true'
);
bad_args(
    'parent display huge', '--parent-display',
    ':123456',             '--parent-xauth',
    '/tmp/a',              '--',
    '/bin/true'
);
bad_args(
    'relative parent xauth', '--parent-display',
    ':0',                    '--parent-xauth',
    '.Xauthority',           '--',
    '/bin/true'
);
bad_args(
    'parent xauth NUL', '--parent-display',
    ':0',               '--parent-xauth',
    "/tmp/a\0b",        '--',
    '/bin/true'
);
bad_args(
    'parent xauth too long', '--parent-display',
    ':0',                    '--parent-xauth',
    '/' . 'a' x 5000,        '--',
    '/bin/true'
);
...
bad_args( 'display with colon', @$base, '--display', ':32', '--', '/bin/true' );
...
bad_args( 'unveil without perm', @$base, '--unveil', '/tmp', '--',
    '/bin/true' );
bad_args( 'unveil bad perm', @$base, '--unveil', '/tmp:z', '--', '/bin/true' );
bad_args( 'unveil perm with colon',
    @$base, '--unveil', '/a:b:c', '--', '/bin/true' );
bad_args( 'unveil relative path',
    @$base, '--unveil', 'tmp:r', '--', '/bin/true' );
bad_args( 'unveil empty path', @$base, '--unveil', ':r', '--', '/bin/true' );
bad_args(
    'unveil too many entries',
    @$base, '--unveil', join( ',', map { "/p$_" . ':r' } 1 .. 70 ),
    '--',   '/bin/true'
);
bad_args( 'missing parent display',
    '--parent-xauth', '/tmp/a', '--', '/bin/true' );
bad_args( 'missing parent xauth', '--parent-display', ':0', '--', '/bin/true' );

# Cookie and session values must not be accepted through arguments
bad_args( 'no cookie option', @$base, '--cookie', 'cafe', '--', '/bin/true' );
bad_args( 'no session option',
    @$base, '--session', '/var/run/x', '--', '/bin/true' );

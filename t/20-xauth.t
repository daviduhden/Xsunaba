...
my ( $FAMILY_LOCAL, $FAMILY_WILD ) = (
    Xsunaba::Helper::FAMILY_LOCAL(),
    Xsunaba::Helper::FAMILY_WILD(),
);

# --- Record packing and parsing round trip -------------------------
my $cookie = 'a1b2c3d4e5f60718293a4b5c6d7e8f90';
my $records = Xsunaba::Helper::xauth_records_for(
    'test.example.org', '32', $cookie,
);
...
my $all = join '', map {
    Xsunaba::Helper::xauth_record_bytes($_)
} @$records;
...
my $parent_bytes = join '', map {
    Xsunaba::Helper::xauth_record_bytes($_)
} @$parent;

my $matched =
  Xsunaba::Helper::extract_parent_auth(
    Xsunaba::Helper::parse_xauth_bytes($parent_bytes),
    ':0',
  );
...
for my $m (@$matched) {
    ...
    is( unpack( 'H*', $m->{data} ),
        unpack( 'H*', $cookie0 ), 'extracted cookie' );
}
...
dies_ok {
    Xsunaba::Helper::extract_parent_auth(
        Xsunaba::Helper::parse_xauth_bytes($bad_bytes),
        ':0',
    );
}
...
dies_ok {
    Xsunaba::Helper::xauth_record_bytes(
        { family => 256, address => 'x' x 5000, number => '0',
          name => 'n', data => '' }
    );
}
